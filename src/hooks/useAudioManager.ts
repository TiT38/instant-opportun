...
interface AudioFrequency {
  base: number;
  beat: number;
  name: string;
}

const frequencies: Record<string, AudioFrequency> = {
...
// Mapping des sessions vers leurs fréquences par défaut
const sessionFrequencies: Record<string, string> = {
  sos: 'theta',           // SOS Stress : Theta
  focus: 'alpha',         // Focus : Alpha
  recovery: 'theta',      // Récup : Theta
  transition: 'coherence', // Transition : 0,1 Hz
  scan: 'delta',          // Scan Corporel : Delta
...
export const useAudioManager = () => {
  ...
  // Obtenir la fréquence par défaut pour la session courante
  const getDefaultFrequency = (): string => {
  ...
  const startAudio = (frequency?: string) => {
    if (!audioSettings.enabled || isPlayingRef.current) return;

    // Utiliser la fréquence par défaut si aucune n'est spécifiée
    ...
    try {
      ...
      // Volume recommandé : 20-30% du volume total
      const recommendedVolume = audioSettings.volume * 0.25; // 25% comme compromis
  ...
  const playGong = (type: 'inhale' | 'exhale' = 'inhale') => {
    ...
    try {
      // Créer les oscillateurs pour le gong
      ...
      // Fréquences du gong
      const baseFreq = type === 'inhale' ? 261.63 : 220.00; // Do4 pour inspire, La3 pour expire
      ...
      // Volume recommandé : 10-20% du volume total
      const recommendedGongVolume = audioSettings.gongVolume * 0.15; // 15% comme compromis
